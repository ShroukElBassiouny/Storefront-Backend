...
export class ProductStore {
  async getListOfAllProducts(): Promise<Products[] | undefined> {
    try {
  ...
  async getProductUsingId(id: String): Promise<Product | undefined> {
    try {
      const conn = await Client.connect()
  ...
  async getTheMostPopularProducts(): Promise<Products[] | undefined> {
    try {
      const conn = await Client.connect()
      const sql =
